...
_aicall_extract_blocks() {
    ...
    local current_block=""
    local reading=false
    local found_blocks=false
    
    while IFS= read -r line; do
        # Trim whitespace and check for ```bash
        if [[ "${line## }" =~ ^'```bash'$ ]]; then
            reading=true
            current_block=""
            continue
        # Trim whitespace and check for closing ```
    ...
    # Return special marker if no blocks found
    if [[ "$found_blocks" == false ]]; then
        echo "NO_BLOCKS_FOUND"
        return
    fi
...
EOF
    ...
    # Form system prompt
    local openrouter_system_prompt="You are a Linux system administrator assistant. Always include at most ONE code block with code inside in your responses, formatted as \`\`\`bash ... \`\`\`, if you can. Your output should be concise and practical. You are helping on the following system:
...
$sys_info"
    ...
    # Extract code blocks with special handling for no blocks
    local blocks_output=$(_aicall_extract_blocks "$response")
    
    if [[ "$blocks_output" == "NO_BLOCKS_FOUND" ]]; then
        print ""
        return
    fi
    
    local IFS='|'
    local -a code_blocks=("${(@s:|:)blocks_output}")
    
    if [[ ${#code_blocks} -eq 1 ]]; then
        print "Execute? [y/N] "
        exec < /dev/tty
        read -k 1 confirm
...
zle -A backward-delete-char orig-backward-delete-char
zle -A accept-line orig-accept-line

typeset -g ORIG_SEND_BREAK=$(bindkey '^C' | awk '{print $2}')
typeset -g ORIG_TRIGGER=$(bindkey ']' | awk '{print $2}')
...
# Create widgets
zle -N accept-line special-mode-handler
zle -N special-mode-interrupt
zle -N special-mode-trigger
zle -N backward-delete-char normal-backspace
